...
def train_generator_ma(mean_fs, var_fs, num_layers, layer_weights):
    ...
    for step in range(2000):
        ...
        generated_features = [fe.view(bs, -1) for fe in feature_extractor(
            normalize_batch(generator(torch.tensor(np.random.normal(0, 1, (bs, 100))).float())))]
        batch_delta_m = []
        batch_delta_c = []
        for i in range(num_layers):
        ...
        with torch.no_grad():
            for i in range(num_layers):
                mt_means[i], ut_means[i], adam_mean = update_moving_averages(running_means_dt[i] - batch_delta_m[i],
                                                                             mt_means[i], ut_means[i], step + 1)
                mt_covs[i], ut_covs[i], adam_cov = update_moving_averages(running_covs_dt[i] - batch_delta_c[i],
                                                                          mt_covs[i], ut_covs[i], step + 1)
...
def visualize_sample(generator, name):
    ...
    with torch.no_grad():
        ...
        for i in range(16):
            pred = generator(torch.tensor(np.random.normal(0, 1, (1, 100))).float())
            pred = pred * 255.0
            im = pred.clamp(0, 255).squeeze().numpy()
            im = im.astype("uint8")
            im = np.transpose(im, (1, 2, 0))
            im = Image.fromarray(im).resize((320, 320))
            fname = './gen_res/%d/%s' % (i, name)
